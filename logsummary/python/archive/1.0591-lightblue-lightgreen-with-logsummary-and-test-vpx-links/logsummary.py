...
def GetInfo(testName,hostName):
	...
	for i in range(0, len(driverName)):
		tmpCtrlDetail.append( "Driver: " + driverName[i] + " -" +
							  driverVersion[i] + "   ,   " + "vid:did svid:sdid: " + viddidsvidsdid[i] + "   ,   " + 
	...
	if len(deviceSN) > 0:
		for j in range(0, len(driverName)):	
			if "nvme" in tmpCtrlDetail[j]:
				tmpCtrlDetail[j] = tmpCtrlDetail[j] + "  ,  " + modelNumber[k] + "  ,  " + deviceSN[k] + "  ,  " + firmwareRevision[k] 
				k=k+1
...
def printDiskMapping():
	...
	if chkNumDiskMapping == 1:
		...
		for x in range(diskMappingIndex, len(diskMapping)):
			# all drives w/ SAS or SATA
			if "naa" in diskMapping[x].split("MDs:")[0] and "naa" in diskMapping[x].split("MDs:")[1]:
				print diskMapping[x].split("MDs:")[0].count("naa"), "+",    #count all the drives for SSD (left side)
				print diskMapping[x].split("MDs:")[1].count("naa"),   #count all the drives for MDs (right side)
				print " -> " + diskMapping[x],
			...
			# NVMe for cache and capacity sas or sata	
			if "NVMe" in diskMapping[x].split("MDs:")[0] and "naa" in diskMapping[x].split("MDs:")[1]:
				print diskMapping[x].split("MDs:")[0].count("t10.NVMe"), "+",    #count all the drives for NVMe (left side)
				print diskMapping[x].split("MDs:")[1].count("naa"),   #count all the drives for MDs (right side)
				print " -> " + diskMapping[x],
			...
			# NVMe for cache and capacity NVMe
			if "NVMe" in diskMapping[x].split("MDs:")[0] and "NVMe" in diskMapping[x].split("MDs:")[1]:
				print diskMapping[x].split("MDs:")[0].count("t10.NVMe"), "+",    #count all the drives for NVMe (left side)
				print diskMapping[x].split("MDs:")[1].count("t10.NVMe"),   #count all the drives for NVMe (right side)
				print " -> " + diskMapping[x],
				cache+=diskMapping[x].split("MDs:")[0].count("t10.NVMe")
...
def displaySummary(testName,hostName,test,testVer,deployVMtime,allVMDeployedtime,status,hpComplete):
	...
	print "=ESX Version="
	esxver = cmdline("cat traces/*/stats.html | grep -iE 'VMware ESXi' | awk '{ print $1,$2,$3,$4 }' | sort | uniq ")
	print esxver 
	print "\n\n"
	...
	#allflash and Hybrid tests				
		...
		print "=DiskGroup timeout (workaround)="
...
def collectLogs(filename, testName):
	...
	global controller
	global driverName, driverVersion, viddidsvidsdid, controllerQueueDepth, deviceSN, modelNumber, Model, firmwareRevision
...
# read through the test-vpx file and collect data
	...
	for line in readFile:
		...
		if re.match('(.*)Driver Name         ==>', line):
			driverName.append(line.split("\n")[0].replace('Driver Name         ==>','    Driver Name:').split("\t")[1])

		if re.match("(.*)Driver Version      ==>", line):
			if "b'Version" in line:
				driverVersion.append(line.split("\n")[0].replace(r'Driver Version      ==>	 Version:', '').split("\t")[1])
			else:
				driverVersion.append(line.split("\n")[0].replace('Driver Version      ==>	 Version:', '').split("\t")[0])

		if re.match("(.*)VID:DID SVID:SDID   ==>", line):
			viddidsvidsdid.append(line.split("\n")[0].replace('VID:DID SVID:SDID   ==>', ' VID:DID SVID:SDID:').split("\t")[1])

		if re.match("(.*)QDepth Current:Max  ==>", line):
			controllerQueueDepth.append(line.split("\n")[0].replace('QDepth Current:Max  ==>', '').split("\t")[1])
