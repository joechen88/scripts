...
def GetInfo(testName,hostName):
	...
	if len(firmwareRevision) > 0:
		for i in range(0, len(modelNumber)):

			tmpCtrlDetail.append( modelNumber[i].split("\n")[0] + "\t" + driverName[i].split("\n")[0].replace('Driver Name         ==>','Driver Name:') + 
								  driverVersion[i].split("\n")[0].replace('Driver Version      ==>	 Version:', '   Version:') +
								  viddidsvidsdid[i].split("\n")[0].replace('VID:DID SVID:SDID   ==>', '\t\t VID:DID SVID:SDID:') +
								  controllerQueueDepth[i].split("\n")[0].replace('QDepth Current:Max  ==>', '\t QDepth') +
								  deviceSN[i].split("\n")[0] +
								  firmwareRevision[i].split("\n")[0] )

	elif len(driverVersion) > 0: 

		for i in range(0, len(controllerQueueDepth)):

			tmpCtrlDetail.append( driverName[i].split("\n")[0] + "\t\t" + driverVersion[i].split("\n")[0].replace('Driver Version      ==>	 Version:', '   Version:') +
								  viddidsvidsdid[i].split("\n")[0].replace('VID:DID SVID:SDID   ==>', '\t\t VID:DID SVID:SDID:') +
								  controllerQueueDepth[i].split("\n")[0].replace('QDepth Current:Max  ==>','\t QDepth') ) 
