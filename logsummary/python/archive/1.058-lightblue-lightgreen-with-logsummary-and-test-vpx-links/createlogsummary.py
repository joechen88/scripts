...
#
# pass in all the 'completed' entries for SanityChecking -- will check against HYconfig or AFconfig list, remove tests with a complete status in the list.   
#
...
def writeSummaryToHTML(summary):
    ...
    #    if not, ( no link )
        ...
        TMPsummaryContent = [ testvpx, testVPXstatus, subDirFileRelative, subTestVPXrelative, statsHTML, statsHTMLlink, vmkernelFolder, vmkerneldir, vendorUtilGetinfo[i], vendorUtil, hn ]
    ...
    #
        ...
        if "......COMPLETED" in testVPXcompletion[j] or "RAID1COMPLETED" in testVPXcompletion[j]:
            tmpTestVPX.append(hostname + "-" + testvpx)


    #
    # A corner case where c1  and c2 got a pass however both of them were from the same host
    #   adding hostname-testvpx into tmpTestVPX then remove dupicates
    # 
    for k in range(0, len(set(tmpTestVPX))): 
        sanityResult.append(tmpTestVPX[k])


    #TestSanityCheck will alter HYconfig or AFconfig accordingly.        
