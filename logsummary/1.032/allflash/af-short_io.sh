...
###############################################################################
# 
# Log summary for af-short_io
#
# Functions:
#   Collect a summary for af-short_io test 
#
###############################################################################
...
if [[ -n "$filename" ]]; then
  ...
  FIO1start="$(cat $filename | grep -iE "starting fio runs" | sed '1!d')"
  FIO1end="$(cat $filename | grep -iE "All VM IO threads are completed" | sed '1!d')"
  ...
  FIO2start="$(cat $filename | grep -iE "starting fio runs" | sed '2!d')"
  FIO2end="$(cat $filename | grep -iE "All VM IO threads are completed" | sed '2!d')"
  ...
  echo ""
  echo "	Usage: af-short_io.sh <filename>"
  echo ""
  echo "               example: sh af-short_io.sh test-vpx.vsan.iocert.ctrlr_short_io_c1.log"
  echo ""
fi
