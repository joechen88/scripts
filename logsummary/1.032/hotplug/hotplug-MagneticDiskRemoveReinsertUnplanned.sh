...
###############################################################################
#
# Log summary for hotplug MagneticDiskRemoveReinsertUnplanned test
#
# Function:
#   Collect a summary for hotplug MagneticDiskRemoveReinsertUnplanned test
#
###############################################################################
...
if [[ -n "$filename" ]]; then
  ...
  echo ""
  echo "                                    ========================================================"
  echo "                                        ${green} MagneticDiskRemoveReinsertUnplanned test ${nc}"
  echo "                                    ========================================================"
  echo ""
  echo ""
  ...
  ResyncTime1="$(cat $filename | grep -iE "Found the user prompt" | tail -n 1)"
  ResyncTime2="$(cat $filename | grep -iE "Found all the" | tail -n 1)"
  timediff "$ResyncTime1" "$ResyncTime2"
  echo ""
  echo ""
  ...
  echo ""
  cat $filename | grep -iE -A 1 'Found all the component|expected state absent|expected state active|waiting for user prompt file|simple component state|found the user prompt|Please be ready to remove|reinsert/plug' | grep -v 'Enter function named vmObjectQuery'
  echo ""
  echo ""
  ...
  echo ""
  echo "	Usage: hotplug-MagneticDiskRemoveReinsertUnplanned.sh <filename>"
  echo ""
  echo "               example: sh hotplug-MagneticDiskRemoveReinsertUnplanned.sh test-vpx.vsan.fvt.test.lsom.diskmanagement.MagneticDiskRemoveReinsertUnplanned_RAID1.log"
  echo ""
fi
