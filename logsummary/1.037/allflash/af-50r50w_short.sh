...
###############################################################################
#
# Log summary for af-50r50w short
#
# Functions:
#   Collect a summary for 50r50w short test 
#
###############################################################################
...
if [[ -n "$filename" ]]; then
  ...
  echo ""
  echo "                                    ===========================================" 
  echo "                                          50r50w short test (50mins max)"
  echo "                                    ==========================================="
  echo ""
  echo ""
  ...
  cat $filename | grep -iE 'vsan.iocert.ctrl_50r50w_short...............................COMPLETED' 
  StartTimeIOBlazerOrFIO="$(cat $filename | grep -iE 'Starting IOBlazer|Starting FIO runs' | sed '1,1!d' | cut -d, -f1)"
  EndTimeIOBlazerOrFIO="$(cat $filename | grep -iE 'vsan.iocert.ctrl_50r50w_short...............................COMPLETED' | sed '1,1!d' | cut -d, -f1)"
  echo ""
  ...
  #status
  echo "=Status="
  cat $filename | grep -i 'vsan.iocert.ctrl_50r50w_short...............................COMPLETED ('
  echo ""
  echo ""

else
  echo ""
  echo "	Usage: af-50r50w_short.sh <filename>"
  echo ""
  echo "               example: sh af-50r50w_short.sh test-vpx.vsan.iocert.ctrl_50r50w_short.log"
  echo ""
fi
