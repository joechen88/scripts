...
if [ "$filename" == "test-vpx.vsan.iocert.ctrl_combined_long_c1.log" ] || [ "$filename" == "test-vpx.vsan.iocert.ctrl_combined_long_c2.log" ]; then
  ...
  echo "${blue}=Dynamo, IOBlazer, FIO=${nc}"
  cat $filename | grep -iE  'Running dynamo|running ioblazer|Starting FIO runs|RunFIOTest: All VM IO threads are completed'
  echo""
  echo""
  ...
  echo "${blue}=70r30w 100% Hit rate with checksum test (4 hrs)=${nc}"
