...
###############################################################################
#.
# Log summary for hotplug-diskRemoveReinsertUnplanned test
#
# Functions:
#   Collect a summary for diskRemoveReinsertUnplanned test.
#
###############################################################################
...
if [[ -n "$filename" ]]; then
  ...
  echo ""
  echo "                                    ==============================================="
  echo "                                        ${green} diskRemoveReinsertUnplanned test ${nc}"
  echo "                                    ==============================================="
  echo ""
  echo ""
  ...
  echo ""
  cat $filename | grep -iE -A 1 'Found all the component|expected state absent|expected state active|waiting for user prompt file|simple component state|found the user prompt|Please be ready to remove|reinsert/plug'
#  cat $filename | grep -iE -A 1 'reinsert/plug'
  echo ""
  echo ""
  ...
  # status
  echo "${blue}=status=${nc}"
  cat $filename | grep -iE "RAID1COMPLETED|raid1fail"
  echo ""
  echo ""
