...
###############################################################################
#.
# Log summary for hotplug-SSDRemoveReinsertPlanned.sh test
#
# Functions:
#   Collect a summary for SSDRemoveReinsertPlanned test.
#
###############################################################################
...
if [[ -n "$filename" ]]; then
  ...
  echo ""
  echo "                                    ==============================================="
  echo "                                          ${green} SSDRemoveReinsertPlanned test ${nc}"
  echo "                                    ==============================================="
  echo ""
  echo ""
  ...
  echo ""
  cat $filename | grep -iE -A 1 'Found all the component|expected state degraded|expected state active|waiting for user prompt file|simple component state|found the user prompt|Please be ready to remove|reinsert' | grep -v 'Enter function named vmObjectQuery'
  echo ""
  echo ""
  ...
  echo ""
  echo "	Usage: hotplug-SSDRemoveReinsertPlanned.sh <filename>"
  echo ""
  echo "               example: sh hotplug-SSDRemoveReinsertPlanned.sh test-vpx.vsan.fvt.test.lsom.diskmanagement.SSDRemoveReinsertPlanned_RAID1.log"
  echo ""
fi
