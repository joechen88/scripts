...
def displaySummary(testName, hostName, test, testVer, deployVMtime,
    ...
    #allflash and Hybrid tests
        ...
        if testName == "secureWipe":
            print "=Secure Wipe results="
            for y, x in enumerate(secureWipe):
                print x,
...
def collectLogs(filename, testName):
    ...
    testname_description_map = {
        ...
        "secureWipe": ["AF only - Secure Wipe (12hrs max)"],
...
#			elif re.match(r'^}$', line):
        ...
        if re.match("(.*)(Secure wipe|secure wipe)",
                    line):
            secureWipe.append(line)
...
def main():
    ...
    parser.add_option(
        ...
        "hp-unplanned, getinfo, sharedvmfs, secureWipe, log-compaction")
