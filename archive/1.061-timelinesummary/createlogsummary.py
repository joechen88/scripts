...
def checkMode(testname):
    ...
    tmpHY = ['7day_stress_c1', '7day_stress_c2', 'combined_long_c1', 'combined_long_c2', 'reset_c1', 'reset_c2', '70r30w_long_99phr_enc_c1', '70r30w_long_99phr_enc_c2',
             'diskRemoveReinsertPlanned', 'diskRemoveReinsertUnplanned', 'short_timeout_io_c1', 'short_timeout_io_c2']
    ...
    tmpAF = ['100r0w_long_af_c1', '100r0w_long_af_c2', '70r30w_long_mdCap_af_c1', '70r30w_long_mdCap_af_c2', 'reset_af_c1', 'reset_af_c2',
             ...
             'short_timeout_io_af_c1', 'short_timeout_io_af_c2']
...
def getTestName(tn):
    if tn == "30r70w_short" or tn == "0r100w_short" or tn == "100r0w_short" or tn == "50r50w_short" or tn == "90r10w_short" or tn == "70r30w_short" or tn == "10r90w_short":
        return "ctrl*"
    ...
    if tn == "short_timeout_io_c1" or tn == "short_timeout_io_c2" or tn == "short_timeout_io_af_c1" or tn == "short_timeout_io_af_c2":
        return "ctrlr_short_timeout_io*"

    if tn == "7day_stress_c1" or tn == "7day_stress_c2" or tn == "7day_stress_af_c1" or tn == "7day_stress_af_c2":
    ...
    if tn == "0r100w_long_4k_af_c1" or tn == "0r100w_long_4k_af_c2" or tn == "0r100w_long_64k_af_c1" or tn == "0r100w_long_64k_af_c2":
        return "0r100w_*"

    if tn == "70r30w_long_mdCap_af_c1" or tn == "70r30w_long_mdCap_af_c2" or tn == "70r30w_long_mdCap_enc_af_c1" or tn == "70r30w_long_mdCap_enc_af_c2" or tn == "70r30w_long_50gb_af_c1" or tn == "70r30w_long_50gb_af_c2":
        return "70r30w_*"

    if tn == "data_integrity_af_c1" or tn == "data_integrity_af_c2":
        return "ctrlr_data_integrity_af*"

    if tn == "diskRemoveReinsertPlanned" or tn == "diskRemoveReinsertUnplanned" or tn == "af_diskRemoveReinsertPlanned" or tn == "af_diskRemoveReinsertUnplanned":
        return "*diskRemove*"
...
def writeSummaryToHTML(summary):
    ...
    #
        ...
        if TMPstatsHTML == None:
         ...
         #current dir subtract original directory for statsHTML
            ...
            if testvpx == "7day_stress_c1" or testvpx == "7day_stress_c2" or testvpx == "7day_stress_af_c1" or testvpx == "7day_stress_af_c2":
                  statsHTMLlink = 'dir'
            else:
    ...
    #    if not, ( no link )
        ...
        TMPsummaryContent = [ testvpx, testVPXstatus, subDirFileRelative, subTestVPXrelative, statsHTML, statsHTMLlink, vmkernelFolder, vmkerneldir, vendorUtilGetinfo[i], vendorUtil, hn, DG[i] ]
    ...
    for key, value in flipped.items():
        for i in value:
            ...
            elif i == "100r0w_short" or i == "10r90w_short" or i == "30r70w_short" or i == "50r50w_short" or i == "70r30w_short" or i == "0r100w_short" or i == "90r10w_short":
                sanityResult.append(i)
...
def genLogsummary(currentPath, testname, ts, output):
    ...
    tmpComplete = cmdline("cat " + "'" + currentPath + "'" + "/" + testname + " | grep -iE 'COMPLETED \(|...cleanfail|...........fail|......SETUPFAIL' ")
    ...
    #get DG
    expectedHost = cmdline('cat ' + '"' + currentPath + '"' + "/" + testname + " |  grep -iE 'matched with Configuration' | awk '{print $9}' ")
    cache = cmdline('cat ' + '"' + currentPath + '"' + "/" + testname + " |  grep -iE 'Expected config " + expectedHost.split("\n")[0] + " Cache' | awk '{print $10}' ")
    capacity = cmdline('cat ' + '"' + currentPath + '"' + "/" + testname + " |  grep -iE 'Expected config " + expectedHost.split("\n")[0] + " Capacity' | awk '{print $10}' ")
...
def runLogSummary(currentPath, testname):
	...
	if testname == "test-vpx.vsan.iocert.ctrlr_short_timeout_io_c1.log":
	    genLogsummary(currentPath, "test-vpx.vsan.iocert.ctrlr_short_timeout_io_c1.log", "shortio", "summary-ctrlr_short_timeout_io_c1.txt")

	if testname == "test-vpx.vsan.iocert.ctrlr_short_timeout_io_c2.log":
		genLogsummary(currentPath, "test-vpx.vsan.iocert.ctrlr_short_timeout_io_c2.log", "shortio", "summary-ctrlr_short_timeout_io_c2.txt")

	if testname == "test-vpx.vsan.iocert.ctrlr_short_timeout_io_af_c1.log":
		genLogsummary(currentPath, "test-vpx.vsan.iocert.ctrlr_short_timeout_io_af_c1.log", "shortio", "summary-ctrlr_short_timeout_io_af_c1.txt")

	if testname == "test-vpx.vsan.iocert.ctrlr_short_timeout_io_af_c2.log":
		genLogsummary(currentPath, "test-vpx.vsan.iocert.ctrlr_short_timeout_io_af_c2.log", "shortio", "summary-ctrlr_short_timeout_io_af_c2.txt")
	...
	if testname == "test-vpx.vsan.fvt.test.lsom.diskmanagement.af_diskRemoveReinsertPlanned_RAID1.log":
	    genLogsummary(currentPath, "test-vpx.vsan.fvt.test.lsom.diskmanagement.af_diskRemoveReinsertPlanned_RAID1.log", "hp-planned", "summary-ctrlr_af_diskRemoveReinsertPlanned.txt")

	if testname == "test-vpx.vsan.fvt.test.lsom.diskmanagement.af_diskRemoveReinsertUnplanned_RAID1.log":
	    genLogsummary(currentPath, "test-vpx.vsan.fvt.test.lsom.diskmanagement.af_diskRemoveReinsertUnplanned_RAID1.log", "hp-unplanned", "summary-ctrlr_af_diskRemoveReinsertUnplanned.txt")
